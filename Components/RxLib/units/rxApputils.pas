...
uses
  Windows, Registry, Classes, Controls, Forms, IniFiles, Grids,
  rxVCLUtils;
...
uses
  ...
  rxStrUtils, rxFileUtil, rxPlacemnt;
...
{$ELSE}
  if IniFile is TIniFile then
    WritePrivateProfileString(StrPLCopy(CSection, Section, Length(CSection) - 1),
      StrPLCopy(CIdent, Ident, Length(CIdent) - 1), nil,
      StrPLCopy(CFileName, TIniFile(IniFile).FileName, Length(CFileName) - 1));
{$ENDIF}
end;
