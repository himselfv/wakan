...
begin
  ...
  if Assigned(HookList) then
    if HookList.IndexOf(Self) >= 0 then
      HookList.Remove(Self);
  while FList.Count > 0 do
    RemoveHook(TRxWindowHook(FList.Last));
  FControl := nil;
  FList.Free;
...
begin
  Result := FControlHook;
  if Result <> nil then
    TControlHook(Result).RemoveHook(Self);
  FActive := False;
end;
...
begin
  if Assigned(FControlHook) then
    Result := TControlHook(FControlHook).WinControl
  else
    Result := FControl;
end;
