...
begin
  Result := {$IFDEF RX_D4}FNoIndexItem{$ELSE}FNoIndexItem^{$ENDIF}; // Polaris
end;
...
begin
  ...
  if EnableNoIndex and (List.IndexOf(NoIndexItem) < 0) then
...
procedure TDBIndexCombo.UpdateList;
  ...
  begin
    ...
    begin
      if (Items.Objects[I] <> nil) and (((IdxFields <> '') and
