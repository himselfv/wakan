...
{$IFDEF MSWINDOWS}
function StrToOem(const AnsiStr: string): AnsiString;
...
function XorEncode(const Key, Source: AnsiString): String;
function XorDecode(const Key, Source: AnsiString): String;
...
var
  C: Integer;
  Number: {$IFDEF RX_D4}Cardinal{$ELSE}Longint{$ENDIF};
begin
  if N = 0 then
    Result := '0'
  else
  begin
    Number := {$IFDEF RX_D4}Cardinal(N){$ELSE}N{$ENDIF};
    Result := '';
    while Number > 0 do
    begin
      C := Number mod B;
...
begin
  ...
  if Length(Key) > 0 then
    for I := 1 to Length(Src) do
      Result[I]:=
      {$IFDEF RX_D12}AnsiChar{$ELSE}Chr{$ENDIF}(Byte(Key[1 + ((I - 1) mod Length(Key))])
                                                xor Ord(Src[I]));
end;
...
begin
  ...
  begin
    C := Chr(StrToIntDef('$' + string(Copy(Source, (I * 2) + 1, 2)), Ord(' ')));
    if Length(Key) > 0 then
