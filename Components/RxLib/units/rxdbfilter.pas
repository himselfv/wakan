...
begin
  ...
  begin
    if ((Operator = canEQ) or (Operator = canNE)) and (Field.DataType = ftString) then
    begin
      S := Right^.FData;
      L := Length(S);
      if L <> 0 then
      begin
        ...
        begin
          if not (foNoPartialCompare in FOptions) and (L > 1) and (S[L] = '*') then
          begin
            Delete(S, L, 1);
            PartialLength := L - 1;
          end;
  ...
  case Node^.FKind of
    enField:
      begin
        ...
        SetNodeOp(Result, 0, PutFieldNode(Field));
        SetNodeOp(Result, 1, PutConstNode(fldBOOL, @BoolFalse, SizeOf(WordBool)));
      end;
    enOperator:
...
begin
  ...
  begin
    NextToken;
    Result := FFilter.NewNode(enOperator, canOR, EmptyStr, Result, ParseExpr2);
  end;
end;
...
begin
  ...
  begin
    NextToken;
    Result := FFilter.NewNode(enOperator, canAND, EmptyStr, Result, ParseExpr3);
  end;
end;
...
begin
  ...
  begin
    NextToken;
    Result := FFilter.NewNode(enOperator, canNOT, EmptyStr, ParseExpr4, nil);
  end
  else
    Result := ParseExpr4;
end;
  ...
  if (FFilter.Count > 0) then
    ...
    try
      Check(DbiAddFilter((FDatalink.DataSet as TBDEDataSet).Handle,
        Longint(Self), FPriority, False, pCANExpr(TExprParser(FParser).FilterData),
        nil, Result));
...
begin
  ...
  Check(DbiAddFilter((FDataLink.DataSet as TBDEDataSet).Handle, Longint(Self),
    FuncPriority, False, nil, PFGENFilter(@TRxDBFilter.RecordFilter), Result));
  FDataHandle := TBDEDataSet(FDatalink.DataSet).Handle;
end;
