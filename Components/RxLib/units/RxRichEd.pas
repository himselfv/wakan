...
{$ENDIF}
  ...
  if Source is TRxTextAttributes then
  begin
...
begin
  ...
  if Source is TRxParaAttributes then
  begin
...
begin
  ...
  begin
    ...
    if MkFirst <> nil then
    begin
      if (MkFirst.IsSystemMoniker(Mksys) = 0) and (Mksys = MKSYS_FILEMONIKER) then
...
function TOleUIObjInfo.GetObjectInfo(dwObject: Longint; var dwObjSize:
Longint; var lpszLabel: PChar; var lpszType: PChar; var lpszShortType: PChar;
  var lpszLocation: PChar): HResult;
begin
...
begin
  ...
  if (L >= 2) and (Text[L - 2] = #13) and (Text[L - 1] = #10) then
    ...
    if (RichEditVersion >= 2) and (L >= 1) and (Text[L - 1] = #13) then
      Dec(L);
    ...
    try
      if StreamInfo^.Converter <> nil then
        pcb := StreamInfo^.Converter.ConvertReadStream(StreamInfo^.Stream,
          {$IFDEF RX_D12}TConversionBuffer{$ENDIF}(pBuff), cb);
      if pcb > 0 then
      begin
        pBuff[pcb] := #0;
      ...
      try
        ...
        try
          if (csDesigning in ComponentState) or Value then
            ...
            TRichEditStrings(Lines).Format := sfRichText;
          TRichEditStrings(Lines).Mode := [];
...
begin
  if (RichEditVersion >= 2) and HandleAllocated then
    SendMessage(Handle, EM_SETTEXTMODE,
      TextModes[Plain] or UndoModes[FUndoLimit > 1], 0);
end;
...
begin
  if Assigned(FCallback) then
    with TRichEditOleCallback(FCallback) do
      if Assigned(FDocForm) and IsFormMDIChild(FDocForm.Form) and
