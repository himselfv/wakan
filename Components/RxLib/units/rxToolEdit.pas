...
begin
  Result := True;
{$IFDEF RX_D4}
  if Editor.UseRightToLeftAlignment then
...
{$ENDIF RX_D4}
  ...
  DC := Message.DC;
  if DC = 0 then
    DC := BeginPaint(Editor.Handle, PS);
  ACanvas.Handle := DC;
  try
    ...
    begin
      ...
      if PopupVisible then
        ALeft := {$IFDEF RX_D10}Margins.Left{$ELSE}MyMargins.X{$ENDIF}
      else
      begin
        ...
        case AAlignment of
          ...
          taRightJustify:
            ALeft := ClientWidth - ButtonWidth - AWidth -
              {$IFDEF RX_D10}Margins.Left{$ELSE}MyMargins.X{$ENDIF} - 2;
          else
...
{$IFDEF RX_D4}
      if SysLocale.MiddleEast then
        UpdateTextFlags;
{$ENDIF}
      TextRect(R, ALeft, {$IFDEF RX_D10}Margins.Top{$ELSE}MyMargins.Y{$ENDIF}, S);
    end;
  finally
    ACanvas.Handle := 0;
    if Message.DC = 0 then
      EndPaint(Editor.Handle, PS);
  end;
end;
...
var
  AValue: {$IFDEF WIN32}Variant{$ELSE}string{$ENDIF};
begin
...
function TCustomComboEdit.GetPopupValue: {$IFDEF WIN32}Variant{$ELSE}string{$ENDIF};
begin
  if FPopup <> nil then
...
procedure TCustomComboEdit.SetPopupValue(
  const Value: {$IFDEF WIN32}Variant{$ELSE}string{$ENDIF});
begin
  if FPopup <> nil then
...
function TCustomComboEdit.AcceptPopup(
  var Value: {$IFDEF WIN32}Variant{$ELSE}string{$ENDIF}): Boolean;
begin
  Result := True;
end;
...
//Polaris
  ...
  if (ssCtrl in Shift)  then
   case Key of
   VK_RETURN: if (Form <> nil) {and Form.KeyPreview} then
     begin
       TCrackWin(Form).KeyDown(Key, Shift);
       Key := 0;
     end;
   VK_TAB: if (Form <> nil){ and Form.KeyPreview} then
     begin
       TCrackWin(Form).KeyDown(Key, Shift);
       Key := 0;
     end;
...
//Polaris
      ...
      if (Value <> ButtonWidth) and ((Assigned(Parent) and (Value < ClientWidth)) or
...
begin
  ...
  if FPopup <> nil then
    if FPopupVisible then
      PopupCloseUp(FPopup, True) else PopupDropDown(True);
end;
...
procedure TFileDirEdit.DoAfterDialog(var FileName: string; var Action: Boolean);
begin
...
// онтроль на недопустимость MinDate>MaxDate
    if (Value <> NullDate) and (FMaxDate <> NullDate) and (Value > FMaxDate) then
      if FDateAutoBetween then
        ...
        raise Exception.CreateFmt(LoadStr(SDateMinLimit),[DateToStr(FMaxDate)]);
    FMinDate := Value;
    UpdatePopup;
    if FDateAutoBetween then
      SetDate(Date);
...
// онтроль на недопустимость MaxDate<MinDate
    if (Value <> NullDate) and (FMinDate <> NullDate) and (Value < FMinDate) then
      if FDateAutoBetween then
        ...
        raise Exception.CreateFmt(LoadStr(SDateMinLimit),[DateToStr(FMinDate)]);
    FMaxDate := Value;
    UpdatePopup;
    if FDateAutoBetween then
      SetDate(Date);
...
begin
  Result := {$IFDEF RX_D4}FTitle{$ELSE}FTitle^{$ENDIF};
end;
...
begin
  if FPopup <> nil then
    Result := TPopupWindow(FPopup).Color
  else
    Result := FPopupColor;
end;
...
begin
  ...
  begin
    if FPopup <> nil then
      TPopupWindow(FPopup).Color := Value;
    FPopupColor := Value;
  end;
end;
...
begin
  if (Key in [VK_PRIOR, VK_NEXT, VK_LEFT, VK_UP, VK_RIGHT, VK_DOWN,
    VK_ADD, VK_SUBTRACT]) and PopupVisible then
  begin
...
function TCustomDateEdit.AcceptPopup(
  var Value: {$IFDEF WIN32}Variant{$ELSE}string{$ENDIF}): Boolean;
var
  D: TDateTime;
begin
...
{$ENDIF}
    ...
    if Result then
      Value := {$IFDEF WIN32}VarFromDateTime(D){$ELSE}FormatDateTime(FDateFormat, D){$ENDIF};
  end;
end;
    ...
    begin
      if ((FMinDate <> NullDate) and (FMaxDate <> NullDate) and
        ...
        raise Exception.CreateFmt(LoadStr(SDateOutOfRange),
          [FormatDateTime(FDateFormat, Value),FormatDateTime(FDateFormat, FMinDate),
      ...
      if ((FMaxDate <> NullDate) and (Value > FMaxDate)) then
        raise Exception.CreateFmt(LoadStr(SDateOutOfMax),
          [FormatDateTime(FDateFormat, Value),FormatDateTime(FDateFormat,FMaxDate)]);
    end;
