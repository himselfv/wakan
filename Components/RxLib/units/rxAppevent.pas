...
begin
  ...
  begin
    ...
    if not TAppEvents(FAppEvents[I]).Chained then
      Exit;
  end;
...
begin
  ...
  for I := FAppEvents.Count - 1 downto 0 do
  begin
    if Assigned(TAppEvents(FAppEvents[I]).FOnException) then
    begin
...
begin
  ...
  begin
    ...
    if not TAppEvents(FAppEvents[I]).Chained or Handled then
      Exit;
  end;
