...
begin
  ...
  if Result = nil then
    Result := {$IFDEF RX_D3}DBTables{$ELSE}DB{$ENDIF}.Session;
end;
  ...
  case FItemType of
    ...
    dtStoredProcs:
      if DataBase.IsSQLBased then
        ...
        DatabaseError(LoadStr(SLocalDatabase));
    dtFiles:
...
begin
{$IFDEF RX_D12}
  Buffer := StrPCopy(AnsiStrAlloc(Length(AnsiString(DatabaseName)) + 1),
...
begin
{$IFDEF RX_D12}
  Buffer := StrPCopy(AnsiStrAlloc(Length(AnsiString(DriverType)) + 1),
