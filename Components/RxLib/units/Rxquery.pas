...
begin
  if not FPatternChanged and not FStreamPatternChanged and (MacroCount = 0) then
    Exit;
...
begin
  try
    ...
    begin
      if FData is TRxQuery then
        TRxQuery(FData).Prepare
      else
    ...
    case FMode of
      ...
      rqOpenOrExec:
        begin
          if FData is TRxQuery then
            TRxQuery(FData).OpenOrExec(True)
          else
            FData.Open;
        end;
    end;
