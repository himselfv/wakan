...
uses
  Windows, SysUtils, Classes, Controls, DB, rxDBUtils;
...
{$IFDEF RX_D12}
  for I := 0 to BlobFieldCount - 1 do
    TBlobDataArray(Buffer + FBlobOfs)[I] := nil;
{$ELSE}
  for I := 0 to BlobFieldCount - 1 do
  ...
  case GetMode of
    ...
    gmCurrent:
      if (FRecordPos < 0) or (FRecordPos >= RecordCount) then
        ...
        if Filtered and not RecordFilter then
          Result := grError;
    gmNext:
...
{$IFDEF RX_D12}
    TBlobDataArrayType(ActiveBuffer + FBlobOfs)[Field.Offset] := nil;
{$ELSE}
    TBlobDataArrayType(ActiveBuffer + FBlobOfs)[Field.Offset] := '';
{$ENDIF}
end;
...
var
  ...
  Pos: Integer;
begin
  Result := {$IFDEF RX_D12}nil{$ELSE}''{$ENDIF};
...
begin
  if FOpened and FCached and (FMode <> bmRead) then
  begin
    FDataSet.SetBlobData(FField, FBuffer, {$IFDEF RX_D12}nil{$ELSE}''{$ENDIF});
    FModified := True;
  end;
end;
