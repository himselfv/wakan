...
begin
  ...
  for i := 0 to ColorsInList - 1 do
    FColorNames.Add(LoadStr(ColorNamesEx[i]));
...
{$IFDEF RX_D3}
      ...
      if (I >= FNumColors) and (ColorValues[I] <> clDefault) and
         (ColorValues[I] <> clNone) then
        Continue;
...
{$IFDEF RX_D3}
      ...
      if ColorValues[I] = clDefault then
        ColorName := SDefault
{$ENDIF}
      else
...
//Polaris
    if coIncludeOther in Options then
      Items.AddObject(LoadStr(SColorCustom), TObject(FCustomColor));
//Polaris
  finally
    Items.EndUpdate;
  end;
...
begin
  ...
  if not Result then
    for i:=0 to ColorsInList-1 do
      if FColorNames[i] <> LoadStr(ColorNamesEx[i]) then
      begin
        Result := True;
        exit;
      end;
...
begin
  ...
  case Message.NotifyCode of
   CBN_SELENDOK
   {CBN_SELCHANGE}: if FInternalDropDown then
   begin
     B := FInternalDropDown;
  ...
  case Message.NotifyCode of
    CBN_SELENDCANCEL,
    CBN_KILLFOCUS,
    CBN_CLOSEUP: FInternalDropDown := False;
    CBN_DROPDOWN: FInternalDropDown := True;
    CBN_SELENDOK: if IValue = (FNumColors + Byte(coIncludeDefault in FOptions)+
                               Byte(coIncludeNone in FOptions)) then
                    FInternalDropDown := False;
  end;
end;
