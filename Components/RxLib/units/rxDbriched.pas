...
{$ENDIF}
  if FMemoLoaded then
  begin
    if CharInSet({$IFDEF RX_D12}AnsiKey{$ELSE}Key{$ENDIF}, [#32..#255]) and
...
var
  Stream: TBlobStream;
begin
  if FMemoLoaded or (FDataLink.Field = nil) or not (FDataLink.Field is TBlobField) then
    Exit;
  FUpdating := True;
  try
...
begin
  if FMemoLoaded and not FUpdating and EditCanModify then
    Change;
  inherited;
end;
