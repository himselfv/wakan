...
{$IFNDEF RX_D12}
    procedure GetChildren(Proc: TGetChildProc {$IFDEF RX_D3}; Root: TComponent {$ENDIF}); override;
{$ENDIF}
...
begin
  ...
  begin
    ...
    if (List <> nil) and (Index >= 0) and (Index < List.Count) then
      Result := List[Index];
  end;
end;
...
begin
  Result := 0;
  if (Section >= 0) and (Section < FSections.Count) and
     (FSections[Section] <> nil) then
...
begin
  if Owner <> nil then
    Section := TSpeedbarSection.Create(Owner)
  else
    Section := TSpeedbarSection.Create(Self);
      ...
      begin
        ...
        begin
          Item.Left := Max(StrToIntDef(ExtractWord(2, S, Delims), Item.Left),
            FOffset.X);
          Item.Top := Max(StrToIntDef(ExtractWord(3, S, Delims), Item.Top), FOffset.Y);
          Item.Visible := True;
        end;
      end;
    end;
...
begin
  ...
  if Window <> nil then
    if Window is TSpeedBar then
      Result := Window as TSpeedBar;
end;
