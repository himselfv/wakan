...
uses {$IFNDEF RX_D12}rxStrUtils,{$ENDIF} RxTConst;
...
begin
  ...
  if CharInSet(C, ['+', '-', '0'..'9', '.', DecimalSeparator]) then
  begin
    if CharInSet(C, ['.', DecimalSeparator]) then
      TmpStr := '0' + '.'
    else
      TmpStr := C;
    NextChar;
    ...
    if (Length(TmpStr) = 1) and CharInSet(TmpStr[1], ['+', '-']) and
      CharInSet(C, ['.', DecimalSeparator]) then
      TmpStr := TmpStr + '0';
    while CharInSet(C, ['0'..'9', '.', 'E', 'e', DecimalSeparator]) do
    begin
...
begin
  ...
  begin
    ...
    if CharInSet(FParseText[1], ['-', '+']) then
