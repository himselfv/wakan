...
begin
  ...
  FObjectInstance := {$IFDEF RX_D6}Classes.{$ENDIF}MakeObjectInstance(WndProc); // Polaris
  FDesktopRoot := True;
...
begin
  if FObjectInstance <> nil then
    {$IFDEF RX_D6}Classes.{$ENDIF}FreeObjectInstance(FObjectInstance);  // Polaris
  inherited Destroy;
end;
...
begin
  if (Message.wParam = HelpButtonId) and (LongRec(Message.lParam).Hi = BN_CLICKED)
...
begin
  ...
  begin
    CopyMoveFileShell(FileName, DestName, not OverwriteReadOnly, False, False);
    Exit;
  end;
  Destination := DestName;
