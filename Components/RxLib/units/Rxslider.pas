...
begin
  ...
  if (soShowFocus in Options) and FFocused and not (csDesigning in ComponentState) then
  begin
    R := SliderRect;
    InflateRect(R, -2, -2);
...
begin
  ...
  if (Value * 2 < MaxSize) and (Value <> FEdgeSize) then
  begin
    FEdgeSize := Value;
    Sized;
  end;
...
begin
  ...
  begin
    FMinValue := Min;
    FMaxValue := Max;
    if not (csReading in ComponentState) and (Min + Increment > Max) then
      FIncrement := Max - Min;
    if (soShowPoints in Options) then
      Invalidate;
    Self.Value := FValue;
