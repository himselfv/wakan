...
begin
  S := '';
{$IFDEF RX_D12}
  AnsiKey:= AnsiChar(BytesOf(Key)[0]);
{$ENDIF}
  if (FDisplayField <> nil) {and (FDisplayField.DataType = ftString)} then
    case {$IFDEF RX_D12}AnsiKey{$ELSE}Key{$ENDIF} of
      #9, #27: FSearchText := '';
      AnsiChar(VK_BACK), #32..#255:
        if CanModify then
        begin
          if not FPopup then
          begin
...
begin
  ...
  begin
    ...
    if Max <> MaxPos then
      SetScrollRange(Handle, SB_VERT, 0, Max, True {False});
    if CurPos <> Pos then
...
begin
  inherited Create(AOwner);
  if AOwner is TRxLookupControl then
    FCombo := TRxLookupControl(AOwner);
...
begin
  ...
  begin
    if CharInSet({$IFDEF RX_D12}AnsiChar(BytesOf(Key)[0]){$ELSE}Key{$ENDIF}, [#32..#255]) then
    begin
      DropDown;
      if FListVisible then
        FDataList.KeyPress(Key);
...
begin
  if Text = FDisplayEmpty then inherited
    Text := Value;
end;
...
begin
  ...
  try
    S := Text;
    if TPopupDataWindow(FPopup).SearchText(S) then
    begin
      Len := Length(Text);
      Text := TPopupDataWindow(FPopup).DisplayValue;
      SelStart := Len;
