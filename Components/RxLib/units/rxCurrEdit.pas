...
begin
  ...
  for I := 1 to Length(Value) do
    if not CharInSet(Value[I], [DecimalSeparator, '-', '+', '0'..'9', 'e', 'E']) then
      Exit;
  Result := TextToFloat(StrPLCopy(Buffer, Value, Length(Buffer) - 1),
    RetValue, fvExtended);
end;
...
begin
  if PopupVisible and CharInSet(UpCase(Key), ['0'..'9', DecimalSeparator, '.', ',',
    '+', '-', '*', '/', '_', '=', 'C', 'R', 'Q', '%', #8, #13] - [ThousandSeparator]) then
  begin
