...
begin
  ...
  if err<>'' then Application.MessageBox(pchar(_l('#00321^eCannot register dictionary ^cNepoda¯ilo se zaregistrovat slovnÌk ')+pname+#13#13+err),pchar(_l('#00020^eError^cChyba')),MB_ICONERROR or MB_OK);
end;
...
begin
  ...
  if err<>'' then Application.MessageBox(pchar(_l('#00322^eCannot build dictionary ^cNepoda¯ilo se vytvo¯it slovnÌk ')+pname+#13#13+err),pchar(_l('#00020^eError^cChyba')),MB_ICONERROR or MB_OK);
end;
...
begin
  ...
  pd:=SMMessageDlg(_l('#00323^eDictionary loading^cNahr·v·nÌ slovnÌku'),_l('#00324^eLoading dictionary ^cNahr·v·m slovnÌk ')+name+'...');
  try
    ...
    Application.MessageBox(pchar(_l('#00325^eCannot load dictionary ^cNepoda¯ilo se nahr·t slovnÌk ')+pname+#13#13+(ExceptObject as Exception).Message),pchar(_l('#00020^eError^cChyba')),MB_ICONERROR or MB_OK);
  end;
  demandloaded:=true;
  pd.Free;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      s:=textptr;
      if length(s)>64000 then s:=_l('#00342^cP¯Ìliö mnoho dat.^eToo large data.');
      clip:=UnicodeToHex(s);
      cliptrans:='';
